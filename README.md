...
_One of Sentinel's built-in agents (`~/Documents/lab/active/sentinel_fork/agents/chat_agent/`). Split out into its own project on 2026-09-14 — see the parent project's README.md for how Sentinel's agent roster fits together._

`key: chat` · class: `agents/chat_agent/__init__.py → ChatAgent` · panel: standard `normal_panel` (no custom panel) · handler: `send_prompt()`
...
| Location | Role |
|---|---|
| `agents/chat_agent/__init__.py` | `ChatAgent` — message builder. |
